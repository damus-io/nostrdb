                              ...
                              unsigned char *out, size_t *outlen)
{
	...
	int range_count = 0;        /* Track range index for diagnostic output */
	...
	while (remaining > 0) {
		...
		if (consumed == 0) {
			fprintf(stderr, "ndb_negentropy: decode failed at range %d, "
		...
		case NDB_NEG_SKIP: {
			...
			if (written == 0) {
				fprintf(stderr, "ndb_negentropy: SKIP encode failed at "
					"range %d, offset=%zu, buflen=%zu\n",
		...
		case NDB_NEG_FINGERPRINT: {
			...
			if (memcmp(our_fp, in_range.payload.fingerprint, 16) == 0) {
				...
				written = ndb_negentropy_range_encode(
					...
					&out_range, &prev_ts_out);
				if (written == 0) {
					fprintf(stderr, "ndb_negentropy: FP match SKIP encode "
						"failed at range %d, offset=%zu\n",
				...
				if (our_count <= (size_t)neg->idlist_threshold) {
					...
					if (our_count > 0) {
						...
						if (id_buf == NULL) {
							fprintf(stderr, "ndb_negentropy: malloc failed "
								"at range %d, count=%zu\n",
					...
					if (written == 0) {
						fprintf(stderr, "ndb_negentropy: IDLIST encode "
							"failed at range %d, count=%zu, offset=%zu\n",
					...
					for (int s = 0; s < split_count && split_lower < upper_idx; s++) {
						...
						written = ndb_negentropy_range_encode(
							...
							&out_range, &prev_ts_out);
						if (written == 0) {
							fprintf(stderr, "ndb_negentropy: FP split "
								"encode failed at range %d split %d, "
								"offset=%zu\n",
		...
		case NDB_NEG_IDLIST: {
			...
			written = ndb_negentropy_range_encode(
				...
				&out_range, &prev_ts_out);
			if (written == 0) {
				fprintf(stderr, "ndb_negentropy: IDLIST response SKIP "
					"encode failed at range %d, offset=%zu\n",
					range_count, out_offset);
				return 0;
			}
		...
		case NDB_NEG_IDLIST_RESPONSE: {
			...
			written = ndb_negentropy_range_encode(
				...
				&out_range, &prev_ts_out);
			if (written == 0) {
				fprintf(stderr, "ndb_negentropy: IDLIST_RESPONSE SKIP "
					"encode failed at range %d, offset=%zu\n",
					range_count, out_offset);
				return 0;
			}
