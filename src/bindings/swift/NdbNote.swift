...
class NdbNote: Encodable, Equatable, Hashable {
    ...
    func to_owned() -> NdbNote {
        ...
        var note_ptr = ndb_note_ptr()
        memcpy(buf, &note_ptr.ptr, self.count)

        return NdbNote(note: note_ptr, size: self.count, owned: true, key: self.key)
    }
    ...
    static func owned_from_json_cstr(json: UnsafePointer<CChar>, json_len: UInt32, bufsize: Int = 2 << 18) -> NdbNote? {
        ...
        return NdbNote(note: note, size: Int(len), owned: true, key: nil)
    }
